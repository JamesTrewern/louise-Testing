...
%!      move(+Id,+Coordinates,-Move) is det.
%
...
%
move(maze(Id,Dims,Ms),X/Y,M):-
        step_up([maze(Id,Dims,Ms),X/Y],M).
move(maze(Id,Dims,Ms),X/Y,M):-
        step_down([maze(Id,Dims,Ms),X/Y],M).
move(maze(Id,Dims,Ms),X/Y,M):-
        step_right([maze(Id,Dims,Ms),X/Y],M).
move(maze(Id,Dims,Ms),X/Y,M):-
        step_left([maze(Id,Dims,Ms),X/Y],M).


%!      step_up(+Start,-Step) is det.
%
...
%
step_up([maze(Id,Dims,Ms),X/Y],step_up([Id,X/Y,T,Vs],[Id,X_/Y_,T_,[u|Vs]])):-
...
%!      step_down(+Start,-Step) is det.
%
%       Generate a Step down from a Start location.
%
%       As step_up/2 but moves down.
%
step_down([maze(Id,Dims,Ms),X/Y],step_down([Id,X/Y,T,Vs],[Id,X_/Y_,T_,[d|Vs]])):-
        step(X/Y,-,0/1,Ms,Dims,X_/Y_)
        ,map_location(X/Y,T,Ms,Dims,true)
        ,map_location(X_/Y_,T_,Ms,Dims,true)
        ,Vs = '$VAR'('Vs').
...
%!      step_left(+Start,-Step) is det.
%
%       Generate a Step to the left of a Start location.
%
%       As step_up/2 but moves left.
%
step_left([maze(Id,Dims,Ms),X/Y],step_left([Id,X/Y,T,Vs],[Id,X_/Y_,T_,[l|Vs]])):-
...
%!      step_right(+Start,-Step) is det.
%
%       Generate a Step tot he right of a Start location.
%
%       As step_up/2 but moves right.
%
step_right([maze(Id,Dims,Ms),X/Y],step_right([Id,X/Y,T,Vs],[Id,X_/Y_,T_,[r|Vs]])):-
        step(X/Y,+,1/0,Ms,Dims,X_/Y_)
        ,map_location(X/Y,T,Ms,Dims,true)
        ,map_location(X_/Y_,T_,Ms,Dims,true)
        ,Vs = '$VAR'('Vs').
